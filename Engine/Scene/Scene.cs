...
public class Scene
{
    ...
    /// <summary>
    /// Duplicates an entity, copying all its components to a new entity instance.
    /// Uses the ComponentRegistry pattern to support any component type without hardcoding.
    /// </summary>
    /// <param name="entity">The entity to duplicate.</param>
    /// <returns>The newly created entity with cloned components.</returns>
    /// <remarks>
    /// This method automatically handles all component types registered in the ComponentRegistry.
    /// New component types are automatically supported without modifying this method.
    /// Components are cloned using registered cloners or shallow copy fallback.
    /// </remarks>
    public Entity DuplicateEntity(Entity entity)
    {
        ...
        // Iterate through all components and clone them using ComponentRegistry
        foreach (var component in entity.GetAllComponents())
        {
            var clonedComponent = ComponentRegistry.Clone(component);

            // Use reflection to call the generic AddComponent method with the correct type
            var componentType = component.GetType();
            var addComponentMethod = typeof(Entity)
                .GetMethod(nameof(Entity.AddComponent), new[] { componentType });
